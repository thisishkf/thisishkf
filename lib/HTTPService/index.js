 ...
 */
const httpConfigParser = function (method, uriParser, params = null) {
 ...
 */
const httpRequest = function (options, secure = false, callback) {
    try {
        let data = '';
        ...
        let req = reqFunction(options, (res) => {
            ...
            Logger.debug(`httpRequest ${options.host + options.path} statusCode: ${res.statusCode}`);
            res.on('data', (chunk) => {
                data += chunk;
            });
            res.on('end', () => {
